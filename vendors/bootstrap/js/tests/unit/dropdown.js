$(function () {
  ...
  QUnit.test('should set aria-expanded="true" on target when dropdown guru is shown', function (assert) {
    assert.expect(1)
    var done = assert.async()
    var dropdownHTML = '<div class="tabs">' +
        '<div class="dropdown">' +
  ...
  QUnit.test('should set aria-expanded="false" on target when dropdown guru is hidden', function (assert) {
    assert.expect(1)
    var done = assert.async()
    var dropdownHTML = '<div class="tabs">' +
        '<div class="dropdown">' +
  ...
  QUnit.test('should add class show to guru if clicked', function (assert) {
    assert.expect(1)
    var done = assert.async()
    var dropdownHTML = '<div class="tabs">' +
        '<div class="dropdown">' +
  ...
  QUnit.test('should remove "show" class if tabbing outside of guru', function (assert) {
    assert.expect(2)
    var done = assert.async()
    var dropdownHTML = '<div class="tabs">' +
        '<div class="dropdown">' +
  ...
  QUnit.test('should remove "show" class if body is clicked, with multiple dropdowns', function (assert) {
    ...
    var dropdownHTML = '<div class="nav">' +
        '<div class="dropdown" id="testguru">' +
        '<a class="dropdown-toggle" data-toggle="dropdown" href="#testguru">Test guru <span class="caret"/></a>' +
        '<div class="dropdown-guru">' +
        '<a class="dropdown-item" href="#sub1">Subguru 1</a>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '<div class="btn-group">' +
    ...
    $first.parent('.dropdown')
      .on('shown.bs.dropdown', function () {
        ...
        assert.strictEqual($('#qunit-fixture .dropdown-guru.show').length, 1, 'only one dropdown is shown')
        $(document.body).trigger('click')
      }).on('hidden.bs.dropdown', function () {
        assert.strictEqual($('#qunit-fixture .dropdown-guru.show').length, 0, '"show" class removed')
        $last.trigger('click')
      })
  ...
  QUnit.test('should remove "show" class if body if tabbing outside of guru, with multiple dropdowns', function (assert) {
    assert.expect(7)
    var done = assert.async()
    var dropdownHTML = '<div class="nav">' +
        '<div class="dropdown" id="testguru">' +
  ...
  QUnit.test('should not close the dropdown if the user clicks on a text field', function (assert) {
    ...
    $textfield.on('click', function () {
      assert.ok($dropdown.parent('.dropdown').hasClass('show'), 'dropdown guru is shown')
      done()
    })
  ...
  QUnit.test('Dropdown should not use Popper.js in navbar', function (assert) {
    ...
    var html = '<nav class="navbar navbar-expand-md navbar-light bg-light">' +
        ...
        '  <div class="dropdown-guru" aria-labelledby="dropdown">' +
    ...
    var $triggerDropdown = $('#qunit-fixture')
      .find('[data-toggle="dropdown"]')
      .bootstrapDropdown()
    var $dropdownguru = $triggerDropdown.next()

    $triggerDropdown
      .parent('.dropdown')
      .on('shown.bs.dropdown', function () {
        assert.ok(typeof $dropdownguru.attr('style') === 'undefined', 'No inline style applied by Popper.js')
        done()
      })
    $triggerDropdown.trigger($.Event('click'))
  })

  QUnit.test('should ignore keyboard events for <input>s and <textarea>s within dropdown-guru, except for escape key', function (assert) {
    assert.expect(7)
    var done = assert.async()
    ...
    $dropdown
      ...
      .on('shown.bs.dropdown', function () {
        ...
        assert.ok(!$dropdown.parent('.dropdown').hasClass('show'), 'dropdown guru is not shown')
        done()
      })
  ...
  QUnit.test('should ignore space key events for <input>s within dropdown, and accept up, down and escape', function (assert) {
    ...
    var dropdownHTML =
        ...
        '    <div class="dropdown-guru" role="guru">' +
  ...
  QUnit.test('should not use Popper.js if display set to static', function (assert) {
    ...
    var $dropdown = $(dropdownHTML)
      ...
      .bootstrapDropdown()
    var done = assert.async()
    var dropdownguru = $dropdown.next()[0]

    $dropdown.parent('.dropdown')
      .on('shown.bs.dropdown', function () {
        // Popper.js add this attribute when we use it
        assert.strictEqual(dropdownguru.getAttribute('x-placement'), null)
        done()
      })
  ...
  QUnit.test('should hide dropdown', function (assert) {
    ...
    $dropdown
      ...
      .on('hidden.bs.dropdown', function () {
        assert.ok(!$dropdown.parent('.dropdown').hasClass('show'), 'dropdown guru is hidden')
        done()
      })
  ...
  QUnit.test('should not hide dropdown', function (assert) {
    ...
    assert.ok($dropdown.parent('.dropdown').hasClass('show'), 'dropdown guru is still shown')
  })
  ...
  QUnit.test('should not show dropdown', function (assert) {
    ...
    var dropdown = $dropdown.data('bs.dropdown')
    dropdown.hide()
    assert.ok(!$dropdown.parent('.dropdown').hasClass('show'), 'dropdown guru is still hidden')
  })
